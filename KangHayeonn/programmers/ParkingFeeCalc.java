...
class Solution {
	...
	for(String key : entranceHours.keySet()) {
		if(entranceHours.containsKey(key)) {
			...
			// entranceHours.remove(key); -> entranceHours.keySet()을 이용해 for문을 돌리고 있어 동시성 오류
		}
	}
	...
	for(String car : carSet) {
	    if(useHours.containsKey(car)) {
		useTime = useHours.get(car);
